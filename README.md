...
**Live:** [meridian-nu-self.vercel.app](https://meridian-nu-self.vercel.app/)
...
- **A plain wallet address is detected and handled as its own case**, not folded into "unknown contract." A single `eth_getCode` check runs before anything else — an EOA gets an honest "this is a wallet, not a contract" instead of misleading contract-risk language about source verification that doesn't apply to it.
- **The allowlist isn't the only path to a verdict, and it isn't the strongest one.** Every check also counts how many *other* Meridian-monitored wallets have actually been flagged (a real Moment, not just an approval) interacting with that exact address — a live query against Meridian's own data, not a list anyone curates by hand. Real incident history outranks everything else, including the allowlist: a curated list is an opinion formed in advance, a wallet that actually got flagged is observed evidence, and evidence wins even over something otherwise vetted.
- Verdict bands, for addresses with no incident history: allowlisted is **safe**; not allowlisted and either young (under 14 days) or unverified is **danger**; not allowlisted but established and verified is **caution**. R1's raw score always adds its not-allowlisted bonus once you're off the allowlist, which would collapse caution and danger into the same band — age and verification are the signals that actually distinguish "not vetted yet" from "actively looks like a drainer."
...
> **Meridian is new, so the incident signal starts sparse.** "No incidents" today mostly means "not enough history yet," not "proven safe" — that's shown honestly in the UI rather than implied as a guarantee. It gets more meaningful as real usage accumulates, by construction, without anyone having to maintain a list.
...
- **Guardrails** — per-rule tier, three of them: **Off** (never fires, no Moment, no email), **Notify** (Moment + email, no in-app action beyond acknowledging or dismissing it), **Confirm** (everything Notify does, plus the one-tap revoke button on the Moment). Confirm is only offered for R1 and R6 — the only rules with a revoke action to actually enable — rather than being shown as a real choice everywhere and quietly doing nothing on the rules it can't affect
...
**Test suite:** 81/81 passing (`npm test`) across Horizon window processing, pattern detection, pricing, Guard (including the incident-override behavior), and Oracle's scoring/explanation layer. `MeridianKeel.sol`: 44/44 deterministic (`forge test`) plus a fuzzed invariant suite (256,000 calls, fund conservation holds throughout), 100% branch coverage.
...
src/
  ...
  components/
    layout/              Page-composition components (Dashboard, Landing, AppHeader)
    ui/                   Shared primitives (Button, Panel, BackLink)
    (Guard, MomentCard, RevokeButton, AuthGate, ... — feature components)
  lib/
...
- **✅ Guard's danger signal comes from real usage, not a curated list.** Real incident history from Meridian's own users outranks the allowlist itself — evidence beats an opinion formed in advance.
- **✅ All six Oracle rules live.** R1–R6 all produce real Moments from real signals — none are wired-but-inert.
- **✅ Confirm-tier revoke, verified on-chain, for R1 and R6.** Not client-trusted: the revoke transaction (`approve(spender, 0)` or `setApprovalForAll(operator, false)`) is fetched and decoded server-side before a Moment resolves. Confirm only appears as a choice on the two rules it actually affects.
