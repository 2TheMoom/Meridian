...
Week 1 in progress: app scaffold, Supabase schema + RLS, wallet connect + SIWE sign-in, wallet registration (with ownership verification), security headers, and the Horizon worker (WebSocket listener + cron fallback route). Not yet built: Oracle rules engine, Timeline UI, Keel actions — see `meridian-mvp-spec.md` section 10 for the build plan.
